...
public class AICar : MonoBehaviour {
	...
	private SoundControllerAI sound = null;
	...
	void  Start (){	
		sound = transform.GetComponent<SoundControllerAI>();
	...
	steer = RelativeWaypointPosition.x / (RelativeWaypointPosition.magnitude) ;
	
	// now we do the same for torque, but make sure that it doesn't apply any engine torque when going around a sharp turn...
	if ( Mathf.Abs( steer ) < 0.5 ) {
		throttle = RelativeWaypointPosition.z / RelativeWaypointPosition.magnitude - Mathf.Abs( steer );
	}else{
		throttle = -1.0f;
	}
		//Debug.Log(throttle);																			
	...
	IEnumerable  StopHandbraking ( float seconds  ){
