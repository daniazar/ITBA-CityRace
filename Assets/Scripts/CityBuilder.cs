...
public class CityBuilder : MonoBehaviour {
	...
	public string BuildingTag =  "city";
	...
	void Start () {
		...
		for(int s = 0; s < csqs; s++) {		
			...
			for(int a = 0; a < csqs; a++) {
				...
				int x = (int)(Random.value * 6 + 6);
				int z = (int)(Random.value * 6 + 6);
			
				bt.Build((int)(c.r * maxBuildingHeight), x, z, new Vector3(0,0,0), windowMaterials[tx]);
				built++;
			}
		}
		...
		//El delay de combine childs es para poder elminar los edificios que pisan la pista
		StartCoroutine (combineChilds());
	
		
	}
	
	
	IEnumerator combineChilds ()
	{
